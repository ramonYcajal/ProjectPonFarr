...
Public Class BaseDeDatos
    'esta clase ya no se usa porque se hace todo en segundo plano con hilos
    'pero se deja en el proyecto por el código que se usó en un principio
    ...
    '    End Try
    'End Sub
    'Public Sub crearStoredProcedures()
    'Dim str As String
    ...
    '   Try
    '       conexion = New MySqlConnection("Server=localhost;database=gestion;Uid=root;Pwd=")
    '       str = "CREATE DEFINER=`root`@`localhost` PROCEDURE `actualizarCuaderno`(
    '                in _id int,
    '                in _datos text,
    ...
    '   Try
    '       conexion = New MySqlConnection("Server=localhost;database=gestion;Uid=root;Pwd=")
    '       str = "CREATE DEFINER=`root`@`localhost` PROCEDURE `actualizarMaterial`( in _id integer,
    ...
    '   Try
    '       conexion = New MySqlConnection("Server=localhost;database=gestion;Uid=root;Pwd=")
    '       str = "CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminarSubcuenta`(in _id int)
    '               begin
    ...
    '   Try
    '       conexion = New MySqlConnection("Server=localhost;database=gestion;Uid=root;Pwd=")
    '       str = "CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarProveedor`(in _nombre varchar(150),
    '                              in _nif varchar (9),
    '                              in _personaContacto varchar (150),
    '                              in _email varchar(150),
    '                              in _observaciones text,
    ...
    '   Try
    '       conexion = New MySqlConnection("Server=localhost;database=gestion;Uid=root;Pwd=")
    '       str = "CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarRed`(in _cliente varchar(150),
    ...
    '    conexion = New MySqlConnection("Server=localhost;database=gestion;Uid=root;Pwd=")
    '    str =
    '            "CREATE PROCEDURE `mostrarTelefonos` (IN `_id` INT, IN `_nombreTabla` VARCHAR(50))
    '        begin 
    '            case _nombreTabla
    '                when 'cliente' then select * from telefonoscliente where id_cliente=_id; 
    '                when 'proveedor' then select * from telefonosproveedor where id_proveedor=_id;
    '            end case;
    '        end"
    '    conexion.Open()
    '    comando = New MySqlCommand(str, conexion)
    ...
    '    conexion = New MySqlConnection("Server=localhost;database=gestion;Uid=root;Pwd=")
    '    str =
    '            "CREATE  PROCEDURE `mostrarUltimoId`(IN `_nombreTabla` VARCHAR(15), OUT `_id` INT)
    '            begin
    '                    Case _nombreTabla
    '               when 'cliente' then
